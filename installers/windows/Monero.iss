; Phiblock Oxygen Orion GUI Wallet Installer for Windows
; Copyright (c) 2017-2020, The Phiblock Project
; See LICENSE
#define GuiVersion GetFileVersion("bin\phiblock-wallet-gui.exe")

[Setup]
AppName=Phiblock GUI Wallet
...
DefaultDirName={commonpf}\Phiblock GUI Wallet
DefaultGroupName=Phiblock GUI Wallet
UninstallDisplayIcon={app}\phiblock-wallet-gui.exe
...
LicenseFile=LICENSE
AppPublisher=The Phiblock Developer Community
AppPublisherURL=https://phiblock.tech
TimeStampsInUTC=yes
...
; Background info:
; This installer indeed asks for admin rights so the Phiblock files can be copied to a place where they have at least
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Phiblock.

Source: {#file AddBackslash(SourcePath) + "ReadMe.htm"}; DestDir: "{app}"; DestName: "ReadMe.htm"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy
Source: "LICENSE"; DestDir: "{app}"; Flags: ignoreversion

; Phiblock GUI wallet exe and guide
Source: "bin\phiblock-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\phiblock-gui-wallet-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; Phiblock CLI wallet
Source: "bin\extras\phiblock-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\phiblock-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; Phiblock wallet RPC interface implementation
Source: "bin\extras\phiblock-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Phiblock daemon
Source: "bin\phiblockd.exe"; DestDir: "{app}"; Flags: ignoreversion

; Phiblock daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "phiblock-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Phiblock blockchain utilities
Source: "bin\extras\phiblock-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\phiblock-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\phiblock-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\phiblock-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\phiblock-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\phiblock-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\phiblock-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\phiblock-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\phiblock-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\phiblock-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\phiblock-gen-ssl-cert.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\phiblock-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitphiblock');
  s := 'The default folder to store the Phiblock blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\phiblock-project\phiblock-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" Phiblock default location
...
begin
  Result := BlockChainDir('') + '\bitphiblock.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "phiblock-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Phiblock daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\phiblockd.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\phiblock-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Phiblock GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\phiblock-wallet-gui.exe";
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\phiblock-gui-wallet-guide.pdf"; IconFilename: "{app}\phiblock-wallet-gui.exe"
...
Name: "{group}\Utilities\Phiblock Daemon"; Filename: "{app}\phiblockd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\phiblock-wallet-cli.exe"; WorkingDir: "{userdocs}\Phiblock\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\Phiblock\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\phiblock-wallet-gui\phiblock-wallet-gui.log"""
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\phiblock-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM phiblockd.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\phiblock-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\phiblock-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\phiblock-project\phiblock-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\phiblock-project\phiblock-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "phiblock:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "phiblock://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "phiblock"; ValueType: "string"; ValueData: "URL:Phiblock Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "phiblock"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "phiblock\DefaultIcon"; ValueType: "string"; ValueData: "{app}\phiblock-wallet-gui.exe,0"
Root: HKCR; Subkey: "phiblock\shell\open\command"; ValueType: "string"; ValueData: """{app}\phiblock-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "phiblockseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "phiblockseed://a.b.c.d:port"
Root: HKCR; Subkey: "phiblockseed"; ValueType: "string"; ValueData: "URL:Phiblock Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "phiblockseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "phiblockseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\phiblock-wallet-gui.exe,0"
Root: HKCR; Subkey: "phiblockseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\phiblock-wallet-gui.exe"" ""%1"""
